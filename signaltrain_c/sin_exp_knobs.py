...
def get_cuda_data(time_series_length, sampling_freq, chooser, knobs0):
    # Generate data
    x = audio.synth_input_sample(np.arange(time_series_length) / sampling_freq, chooser)
    knobs = audio.random_ends(3)-0.5
    [threshold, ratio, attack] = [ i[0]*(1+i[1]) for i in zip(knobs0,knobs) ]
    ...
    knobs_cuda =  torch.autograd.Variable(torch.from_numpy(knobs).cuda(), requires_grad=True).float()
...
def main_compressor(epochs=5000, n_data_points=1, batch_size=20):
    # Data settings
    ...
    # validation data
    x_val_cuda, y_val_cuda, knobs_val_cuda = get_cuda_data(time_series_length, sampling_freq, None, knobs0)
    ...
    for epoch in range(epochs):
        ...
        for data_point in range(n_data_points):
                # get new data
                x_cuda, y_cuda, knobs_cuda = get_cuda_data(time_series_length, sampling_freq, np.random.choice([0]), knobs0)
                ...
                if (data_point % batch_size == 0) or (data_point == n_data_points-1):
                    iterations += 1
                    x_val_cuda, y_val_cuda, knobs_val_cuda = get_cuda_data(time_series_length, sampling_freq, np.random.choice([0]), knobs0)
...
if __name__ == "__main__":
    ...
    main_compressor(epochs=20000, n_data_points=4000)
