...
def synthaudio_generator(seq_size=8192, dtype=np.float32):
    """
    Python generator function for synthetic waveforms via synth_input_sample()
    """
    t = np.linspace(0,1,num=seq_size).astype(dtype)
    sample_type = 7   # hard coding "noisy pluck" for now
    while True:
        clip = synth_input_sample(t,chooser=sample_type)
        yield clip


def apply_effect(x, effect='id'):                # function to be learned
    """  This is where we can define a variety of effects to be learned.
    """
    if ('comp' == effect):
            return compressor(x)
    else:
...
def gen_audio(seq_size=8192*2, chunk_size=8192,  path='Train',
    ...
    while True:             # 'while True' is a common construct for Python generators
        if effect != 'ta':
            X = next(signal_gen)
            Y = apply_effect(X, effect=effect)
        else:
